...
function renderLayout1(cfg, products, root) {
  ...
  const stepMap = {};
  visibleProducts.forEach(p => { const s = p.step || 1; if (!stepMap[s]) stepMap[s] = []; stepMap[s].push(p); });
  const steps = Object.keys(stepMap).map(Number).sort((a, b) => a - b);
  if (!steps.length) { stepMap[1] = visibleProducts; steps.push(1); }
  const stepsHtml = steps.map(step => {
    ...
    const stepLimit    = sp[0]?.step_limit || parseInt(cfg[`step_${step}_limit`]) || null;
    return `
  ...
  const padTop = isMobile ? (cfg.container_padding_top_mobile||16) : (cfg.container_padding_top_desktop||24);
  const padR   = isMobile ? (cfg.container_padding_right_mobile||12) : (cfg.container_padding_right_desktop||24);
  const padBot = isMobile ? (cfg.container_padding_bottom_mobile||80) : (cfg.container_padding_bottom_desktop||80);
  const padL   = isMobile ? (cfg.container_padding_left_mobile||12) : (cfg.container_padding_left_desktop||24);
...
function bindLayout1Logic(cfg, products) {
  ...
  function getTotalQty()      { return Object.values(selected).reduce((s, i) => s + i.qty, 0); }
  function getStepQty(step)   { return Object.values(selected).filter(i => String(i.step) === String(step)).reduce((s, i) => s + i.qty, 0); }
  ...
  function getProductByCardId(cardId) {
    return products.find(p => { const pId = normalizeProductId(p.id); if (pId === cardId) return true; return (p.variants || []).some(v => normalizeVariantId(v.id) === cardId); }) || null;
  }
  ...
  function updateTotals() {
    const items = Object.values(selected); const totalQty = getTotalQty();
    const total = items.reduce((s, i) => s + (toSafeNumber(i.price) * (parseInt(i.qty, 10) || 0)), 0);
    let disc = total; const origEl = document.getElementById('cdo-original-total');
    const discEl = document.getElementById('cdo-discounted-total'); const motivEl = document.getElementById('cdo-motiv');
    if (totalQty >= maxTotal && discountPc > 0) {
      disc = total * (1 - discountPc / 100);
      if (origEl) { origEl.style.display = 'inline'; origEl.textContent = formatMoney(total * 100); }
      ...
      if (motivEl && totalQty < maxTotal) {
        const rem = maxTotal - totalQty; motivEl.style.display = totalQty > 0 ? 'block' : 'none';
        motivEl.textContent = (cfg.preview_motivation_text || cfg.discount_motivation_text || 'Add __REMAINING__ more to unlock discount!').replace(/\{\{remaining\}\}|__REMAINING__/g, rem);
      }
    }
    ...
    const bar = document.getElementById('cdo-progress-bar'); const ptxt = document.getElementById('cdo-progress-text');
    const umsg = document.getElementById('cdo-unlock-msg'); const track = document.getElementById('cdo-progress-track');
    ...
    if (umsg) { if (totalQty >= maxTotal) { umsg.innerHTML = `<span style="color:${cfg.progress_success_color||'#008060'};font-weight:700;">🎉 ${cfg.discount_unlocked_text||'Discount Unlocked!'}</span>`; } else { const rem = maxTotal - totalQty; umsg.textContent = (cfg.discount_motivation_text || 'Add __REMAINING__ more items to unlock the discount!').replace(/\{\{remaining\}\}|__REMAINING__/g, rem); } }
    document.querySelectorAll('.cdo-step-check').forEach(el => { const step = el.dataset.step; const sp = products.filter(p => String(p.step || 1) === String(step)); const limit = sp[0]?.step_limit || parseInt(cfg[`step_${step}_limit`]) || null; el.style.display = (limit && getStepQty(step) >= limit) ? 'inline' : 'none'; });
    const flat = []; Object.values(selected).forEach(item => { for (let q = 0; q < item.qty; q++) flat.push(item); });
    for (let i = 1; i <= maxTotal; i++) { const slot = document.getElementById(`cdo-slot-${i}`); if (!slot) continue; const item = flat[i - 1]; const img = item ? item.image : ''; if (item && img) { slot.innerHTML = `<img class="cdo-preview-image preview-image" src="${img}" style="width:100%;height:100%;object-fit:cover;object-position:center;border-radius:inherit;display:block;">`; slot.style.border = `2px solid ${hl}`; } else { slot.innerHTML = '+'; slot.style.border = '2px dashed rgba(0,0,0,0.2)'; } }
  }
  function updateCardVisuals(card, id) {
    const cardItems = Object.values(selected).filter(i => i.cardId === id || i.id === id); const qty = cardItems.reduce((s, i) => s + i.qty, 0); const qtyEl = card.querySelector('.cdo-qty-value'); if (qtyEl) qtyEl.value = qty; card.style.border = qty > 0 ? `2px solid ${hl}` : `2px solid ${cfg.preview_item_border_color||'#f0f0f0'}`;
    const addBtn = card.querySelector('.cdo-add-btn'); if (addBtn) addBtn.textContent = qty > 0 ? (cfg.product_add_btn_text||'Added') : (cfg.product_add_btn_text||cfg.add_btn_text||'Add');
    const tick = card.querySelector('.cdo-tick'); if (tick) tick.classList.toggle('visible', qty > 0); updateTotals();
  }
  let pendingCard = null; const overlay = document.getElementById('cdo-variant-overlay'); const varTitle = document.getElementById('cdo-variant-title'); const varOptions = document.getElementById('cdo-variant-options'); const varAddBtn = document.getElementById('cdo-variant-add'); const varClose = document.getElementById('cdo-variant-close');
  function openVariantPopup(card, product) {
    pendingCard = card; if (varTitle) varTitle.textContent = product.title;
    if (varOptions) {
      varOptions.innerHTML = ''; (product.variants || []).forEach(v => {
        const btn = document.createElement('button'); btn.type = 'button'; btn.textContent = v.title === 'Default Title' ? product.title : v.title; btn.dataset.variantId = (v.id || '').replace('gid://shopify/ProductVariant/', ''); btn.dataset.price = v.price; btn.dataset.image = v.image || product.image; btn.style.cssText = `padding:8px 16px;border:2px solid #e0e0e0;border-radius:6px;cursor:pointer;font-weight:600;background:#fff;`;
        btn.addEventListener('click', () => { varOptions.querySelectorAll('button').forEach(b => { b.style.borderColor = '#e0e0e0'; b.style.background = '#fff'; }); btn.style.borderColor = hl; btn.style.background = '#f5f5f5'; if (varAddBtn) { varAddBtn.dataset.variantId = btn.dataset.variantId; varAddBtn.dataset.price = btn.dataset.price; varAddBtn.dataset.image = btn.dataset.image; } }); varOptions.appendChild(btn);
      });
      const first = varOptions.querySelector('button'); if (first) first.click();
    }
    if (overlay) overlay.style.display = 'flex';
  }
  if (varClose) varClose.addEventListener('click', () => { if (overlay) overlay.style.display = 'none'; pendingCard = null; });
  if (overlay) overlay.addEventListener('click', e => { if (e.target === overlay) { overlay.style.display = 'none'; pendingCard = null; } });
  if (varAddBtn) {
    varAddBtn.addEventListener('click', () => {
      if (!pendingCard) return; const variantId = varAddBtn.dataset.variantId; const price = parseFloat(varAddBtn.dataset.price); const image = varAddBtn.dataset.image; const id = pendingCard.dataset.id; const step = pendingCard.dataset.step || '1'; const stepLimit = parseInt(pendingCard.dataset.steplimit) || null;
      if (getTotalQty() >= maxTotal) { showToast(limitMsg); return; } if (stepLimit && getStepQty(step) >= stepLimit) { showToast(`Max ${stepLimit} item(s) from this category.`); return; }
      const key = variantId || id; if (!selected[key]) selected[key] = { id: variantId || id, price, image, qty: 1, step, cardId: id }; else selected[key].qty++;
      updateCardVisuals(pendingCard, id); if (overlay) overlay.style.display = 'none'; pendingCard = null;
    });
  }
  document.querySelectorAll('.cdo-card').forEach(card => {
    const id = card.dataset.id; const price = toSafeNumber(card.dataset.price); const image = card.dataset.image; const step = card.dataset.step || '1'; const stepLimit = parseInt(card.dataset.steplimit) || null; const product = getProductByCardId(id);
    const onInc = (overrideVariant) => {
      ...
      if (overrideVariant) {
        ...
        if (!selected[key]) selected[key] = { id: key, price: vPrice, image: vImage, qty: 1, step, cardId: id }; else selected[key].qty++;
    ...
    const onDec = () => { let key = null; if (selected[id]) { key = id; } else { const activeSwatch = card.querySelector('.cdo-variant-swatch.active'); const activeVariantId = activeSwatch ? activeSwatch.dataset.variantId : null; if (activeVariantId && selected[activeVariantId]) key = activeVariantId; if (!key) key = Object.keys(selected).find(k => selected[k].cardId === id); } if (key) { selected[key].qty--; if (selected[key].qty <= 0) delete selected[key]; updateCardVisuals(card, id); } };
...
async function fetchStorefrontProducts(collectionHandle, signal) {
  const res = await fetch(`/collections/${collectionHandle}/products.json?limit=50`, { signal }); if (!res.ok) throw new Error(`Collection API error: ${res.status}`); const data = await res.json();
  return (data.products || []).map(p => ({
    id: `gid://shopify/Product/${p.id}`, title: p.title, handle: p.handle, price: p.variants?.[0]?.price || '0.00', image: p.images?.[0]?.src || '', available: p.available, inventory_quantity:(p.variants || []).reduce((sum, v) => sum + (parseInt(v.inventory_quantity, 10) || 0), 0), collection_handle: collectionHandle,
    variants: (p.variants || []).map(v => ({ id: `gid://shopify/ProductVariant/${v.id}`, title: v.title, price: v.price, available: v.available, inventory_quantity: v.inventory_quantity, image: v.featured_image?.src || null, options: v.option_values || [] }))
  }));
}
...
function renderLayout2(cfg, products, root, template) {
  ...
  const productsCache = new Map(); let activeHandle = initialHandle; let pendingController = null; let requestSerial = 0; const rootName = cfg.collection_title || 'combo'; const panel = root.querySelector('#cdo-products-panel');
  function getSkeletonHtml() { const skeletonCount = Math.max(cols, 3); let html = '<div class="cdo-products-skeleton">'; for (let i = 0; i < skeletonCount; i++) { html += '<div class="cdo-skeleton-card"><div class="cdo-skeleton-img"></div><div class="cdo-skeleton-line"></div><div class="cdo-skeleton-line short"></div></div>'; } html += '</div>'; return html; }
  function renderGridState(html) { const grid = document.getElementById('cdo-products-grid'); if (!grid) return null; grid.innerHTML = html; return grid; }
  ...
  tabElements.forEach(tab => {
    tab.addEventListener('click', () => { const handle = tab.dataset.handle; if (!handle || handle === activeHandle) return; renderTabProducts(handle); });
    tab.addEventListener('keydown', (event) => { const key = event.key; if (!['ArrowRight', 'ArrowLeft', 'Home', 'End', 'Enter', ' '].includes(key)) return; event.preventDefault(); const currentIndex = tabElements.indexOf(tab); let targetIndex = currentIndex; if (key === 'ArrowRight') targetIndex = (currentIndex + 1) % tabElements.length; if (key === 'ArrowLeft') targetIndex = (currentIndex - 1 + tabElements.length) % tabElements.length; if (key === 'Home') targetIndex = 0; if (key === 'End') targetIndex = tabElements.length - 1; if (key === 'Enter' || key === ' ') { const selectedHandle = tab.dataset.handle; if (selectedHandle && selectedHandle !== activeHandle) renderTabProducts(selectedHandle); return; } const targetTab = tabElements[targetIndex]; if (!targetTab) return; setTabVisualState(targetTab.dataset.handle, true); });
  });
  tabs.forEach(tab => { const localList = filterProductsByStock(readLocalCollection(tab.handle), cfg); if (localList.length || tab.handle === 'all') { productsCache.set(tab.handle, localList); } });
  setTabVisualState(initialHandle, false); renderTabProducts(initialHandle);
}
...
function renderLayoutGrid(cfg, products, root) {
  const visibleProducts = filterProductsByStock(products, cfg); const isMobile = window.innerWidth < 768; const padR = isMobile ? (cfg.container_padding_right_mobile||12) : (cfg.container_padding_right_desktop||24); const padL = isMobile ? (cfg.container_padding_left_mobile||12) : (cfg.container_padding_left_desktop||24); const padT = isMobile ? (cfg.container_padding_top_mobile||16) : (cfg.container_padding_top_desktop||24); const padB = isMobile ? (cfg.container_padding_bottom_mobile||80): (cfg.container_padding_bottom_desktop||80); const cols = isMobile ? (cfg.mobile_columns||2) : (cfg.desktop_columns||3); const gap = cfg.products_gap || 20; const gridW= cfg.grid_width || 100;
  root.innerHTML = `<div style="max-width:${cfg.container_width||1200}px;margin:0 auto;background:${cfg.bg_color||'#fff'};color:${cfg.text_color||'#000'};box-sizing:border-box;">${getProgressHtml(cfg)}<div style="padding:${padT}px ${padR}px ${padB}px ${padL}px;">${getBannerHtml(cfg)}${getTitleHtml(cfg)}<div class="${cfg.grid_layout_type === 'slider' ? 'cdo-layout-slider-container' : ''}">${cfg.grid_layout_type === 'slider' ? `<button class="cdo-nav-btn prev" onclick="this.parentNode.querySelector('.cdo-layout-slider').scrollBy({left:-300,behavior:'smooth'})">←</button><button class="cdo-nav-btn next" onclick="this.parentNode.querySelector('.cdo-layout-slider').scrollBy({left:300,behavior:'smooth'})">→</button>` : ''}<div class="${cfg.grid_layout_type === 'slider' ? 'cdo-layout-slider' : ''}" style="${cfg.grid_layout_type === 'slider' ? `width:${gridW}%;margin-top:20px;` : `width:${gridW}%;margin-top:20px;display:grid;grid-template-columns:repeat(${cols},minmax(0,1fr));gap:${gap}px;`}"></div></div></div>${getPreviewBarHtml(cfg)}</div>`;
