...
The Collection Title and Collection Description AI Sparkle actions use a server-side OpenAI call from `/api/suggestions`.

Add these environment variables before running locally or deploying:

```shell
OPENAI_API_KEY=your_openai_api_key
# Optional: defaults to gpt-4.1-mini
OPENAI_SUGGEST_MODEL=gpt-4.1-mini
```

Security note: keep API keys on the server only. The browser never receives the OpenAI key.
